define(['backbone.marionette',
    ...
    function(Marionette, Elements, elements) {
    ...
    return Marionette.Object.extend({
        ...
        // Initialize mucalc with
        //   elements: elemental data
        //   ephoton: photon energy (keV)
        ...
        elementMu: function(options) {
            ...
            if (el) {
                ...
                if (photo_barn === null) photo_barn = this.mcMasterCalc({ fit: mcm.get('nfit') })

                var coh_barn = this.mcMasterCalc({ fit: mcm.get('coherent') })
                var incoh_barn = this.mcMasterCalc({ fit: mcm.get('incoherent') })

                var c = mcm.get('conversion')
                return [coh_barn/c, incoh_barn/c, photo_barn/c, (coh_barn+incoh_barn+photo_barn)/c]
            }
        },

        mcMasterCalc: function(options) {
            var logE = Math.log(this.ephoton)

            var xs = 0
            _.each(options.fit, function(f,i) {
        ...
        hashFromComposition: function(compositionIn) {
        ...
        hashFromBracketlessComposition: function(composition) {
        ...
        weightHash: function(elementHash, multiplier) {
        ...
        hillOrder: function(elementSymbol) {
            var hillOrdered = ['C', 'H',
        ...
        compareHill: function(elementA, elementB) {
